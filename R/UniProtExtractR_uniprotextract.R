## UniProtExtractR. AP 23-12-24.
uniprotextract <- function(my.uniprot.df, map.up=NULL, write.local = FALSE) {
  #### Install necessary packages ####
  list.of.packages <- c("stringr", "stringi", "tibble")
  ...
  #### Write to .csv file and summarize changes if write.local = TRUE else return dataframe up ####
  ## if write.local = TRUE, then this is run
  if (isTRUE(write.local)) {
  ...
  ## This section is always run in Shiny version, run locally by default unless write.local = TRUE, in which case the dataframe up gets written to .csv and is not returned to the R environment.
  return(up)
  }
